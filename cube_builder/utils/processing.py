    ...
    for composite_function in [cube_parts.composite_function]:
        ...
        with db.session.begin_nested():
            ...
            for band in scenes:
                ...
                relative_work_dir_file = Path(str(scenes[band][composite_function])).relative_to(Config.WORK_DIR)
                destination_file = Path(Config.DATA_DIR) / relative_work_dir_file
...
def publish_merge(bands, datacube, tile_id, date, scenes, band_map, reuse_data_cube=None):
    ...
    with db.session.begin_nested():
        ...
        for band in scenes['ARDfiles']:
            ...
            relative_work_dir_file = Path(str(scenes['ARDfiles'][band])).relative_to(Config.WORK_DIR)
            destination_file = Path(Config.DATA_DIR) / relative_work_dir_file
            # Ensure file is writable
            destination_file.parent.mkdir(exist_ok=True)
