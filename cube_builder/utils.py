...
def blend(activity, band_map, build_clear_observation=False):
    ...
    Basically, the blend operation consists in stack all the images (merges) in period and then. The stack is based in
    best pixel image (Best clear ratio)

    For example, a data cube Landsat-8 16 days using function Best Pixel (Stack - STK). The scenes in period 16 days
    from 1/1 to 16/1 found `10/1` and `15/1` scenes as the following:
    ...
    According to Brazil Data Cube User Guide, the best image is 15/1 and worst as 10/1. The result data cube will be:
