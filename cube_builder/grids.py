...
def create_grids(names: List[str], projection, meridian, tile_scale: List[Tuple[int, int]],
    ...
    ref_resolution_x, ref_resolution_y = resolution
    meridian_alber = transform(CRS.from_proj4(src_crs), grid_crs, [meridian], [-12])

    grid_crs_transformer = pyproj.Transformer.from_crs(pyproj.CRS.from_proj4(src_crs), grid_crs)
    wgs_bbox = box(bbox_obj['w'], bbox_obj['s'], bbox_obj['e'], bbox_obj['n'])

    bbox_alber_envelope = shapely_transform(grid_crs_transformer.transform, wgs_bbox).envelope.bounds

    xs = [
        (meridian_alber[0][0] - (tile_width / 2.) * ref_resolution_x),
        (meridian_alber[0][0] + (tile_width / 2.0) * ref_resolution_x),
        meridian_alber[0][0], meridian_alber[0][0], 0.
    ]
    ys = [0, 0., -(tile_height / 2.) * ref_resolution_y, (tile_height / 2.) * ref_resolution_y, 0.]
    out = [xs, ys]
    xmin_center_tile = out[0][0]
    xmax_center_tile = out[0][1]
    ymin_center_tile = out[1][2] + meridian_alber[1][0]
    ymax_center_tile = out[1][3] + meridian_alber[1][0]
    tile_size_x = xmax_center_tile - xmin_center_tile
    tile_size_y = ymax_center_tile - ymin_center_tile

    # Border coordinates of WRS grid
    total_tiles_left = math.ceil(abs((xmin_center_tile - bbox_alber_envelope[0])) / tile_size_x)
    total_tiles_upper = math.ceil(abs((ymax_center_tile - bbox_alber_envelope[3])) / tile_size_y) + 1

    x_min_grid = xmin_center_tile - total_tiles_left * tile_size_x
    y_max_grid = ymax_center_tile + total_tiles_upper * tile_size_y

    # Upper Left is (xl, yu) Bottom Right is (xr,yb)
    xs = [bbox_obj['w'], bbox_obj['e'], meridian, meridian, 0.0]
    ys = [0., 0., bbox_obj['n'], bbox_obj['s'], 0.0]
    out = transform(src_crs, grid_crs, xs, ys, zs=None)
    ...
    for grid_name, (res_x, res_y) in zip(names, tile_scale):
        ...
        tiles, features = _create_tiles(tile_size=(grid_tile_size_x, grid_tile_size_y),
                                        grid_x_min=x_min_grid, grid_y_max=y_max_grid,
                                        bbox=(xl, xr, yb, yu),
                                        srid=srid)
